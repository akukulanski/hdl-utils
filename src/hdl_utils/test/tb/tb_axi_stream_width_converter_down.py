...
from hdl_utils.cocotb_utils.tb_utils import width_converter_down
...
async def run_and_check_result(dut, tb, data_in, user_in, keep_in,
    ...
    expected_data_out = width_converter_down(
        data_in=data_in,
        width_in=tb.data_width_in,
        width_out=tb.data_width_out)
    expected_user_out = width_converter_down(
        data_in=user_in,
        width_in=tb.user_width_in,
        width_out=tb.user_width_out) or [0] * len(expected_data_out)
    expected_keep_out = width_converter_down(
        data_in=keep_in,
        width_in=tb.keep_width_in,
        width_out=tb.keep_width_out)

    # Remove null-packets:
    length = len(expected_data_out)
    for i in reversed(range(length)):
        if expected_keep_out[i] == 0:
            expected_data_out.pop()
            expected_user_out.pop()
            expected_keep_out.pop()
        else:
            break
...
async def tb_write_read_single(dut, burps_in, burps_out):
    tb = Testbench(dut)
    await tb.init_test()
    ...
    test_length = 10
    data_in = [random.getrandbits(tb.data_width_in) for _ in range(test_length)]
    user_in = [random.getrandbits(P_UWI) for _ in range(test_length)]
...
async def tb_write_read_multiple(dut, burps_in, burps_out):
    tb = Testbench(dut)
    await tb.init_test()
...
async def tb_partial_tkeep(dut, burps_in, burps_out):
    tb = Testbench(dut)
    await tb.init_test()
...
async def tb_check_no_clock_wasted(dut):
    ...
    expected_clk_cycles_per_input = tb.scale_factor
...
tf_tb_write_read_single.add_option(
    ("burps_in", "burps_out"),
...
tf_tb_write_read_multiple.add_option(
    ("burps_in", "burps_out"),
    [(False, False), (True, False), (False, True), (True, True)]
)
tf_tb_write_read_multiple.generate_tests()

# Test Final Packet with some null keep bits
tf_tb_partial_tkeep = TestFactory(test_function=tb_partial_tkeep)
tf_tb_partial_tkeep.add_option(
    ("burps_in", "burps_out"),
    [(False, False), (True, False), (False, True), (True, True)]
)
tf_tb_partial_tkeep.generate_tests()
